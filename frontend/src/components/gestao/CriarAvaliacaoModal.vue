  ...
  <div v-if="open" class="modal-overlay" @click.self="$emit('close')">
    <div class="modal">
      <div class="modal-header">
        <h3 class="modal-title">Nova Avaliação de Estágio</h3>
      ...
      <div class="modal-body">
        <div class="field">
          <label class="field-label">Estagiário <span class="req">*</span></label>
          <select v-model="form.estagiarioId" class="field-input">
            <option value="" disabled>Selecione o estagiário</option>
            <option v-for="e in estagiarios" :key="e._id" :value="e._id">
              {{ e.name }} — RG {{ e.rg }}
            </option>
          </select>
          <span v-if="!estagiarios.length" class="field-hint-warn">Nenhum estagiário ativo encontrado.</span>
        </div>
...
const props = defineProps({
  ...
  estagiarios: { type: Array, default: () => [] },
...
const form = reactive({ estagiarioId: '', avaliacao: '', pontoAtencao: '', nota: 5 })

watch(() => props.open, (val) => {
  if (val) Object.assign(form, { estagiarioId: '', avaliacao: '', pontoAtencao: '', nota: 5 })
})

const canSubmit = computed(() => !!form.estagiarioId && !!form.avaliacao.trim())

const notaDesc = computed(() => {
  if (form.nota <= 3) return 'Retirar do estágio'
