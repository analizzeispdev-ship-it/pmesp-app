      ...
      <main class="content">
        <div class="page-card">
          ...
          <div class="card-header">
            <div>
              <h3 class="card-title">Avaliações de Estágio</h3>
              <p class="card-sub">{{ cardSubtitle }}</p>
            </div>
            <button v-if="!isEstagio" class="btn-criar" @click="openCriar">
          ...
          <!-- Filtros (não estagio) -->
          <div v-if="!isEstagio" class="filters-bar">
            ...
            <template v-if="isP1">
              ...
              <div class="filter-group">
                ...
                <select v-model="filters.estagiarioId" class="filter-select">
                  <option value="">Todos estagiários</option>
                  <option v-for="e in store.estagiarios" :key="e._id" :value="e._id">{{ e.name }}</option>
                </select>
              </div>
            </template>
          ...
          >
            ...
            <template #head>
              ...
              <th v-if="!isEstagio">Estagiário</th>
            ...
            <template #body>
              <tr v-for="av in store.avaliacoes" :key="av._id">
                ...
                <td v-if="!isEstagio">
                  <div class="person-cell">
                    <span class="person-name">{{ av.estagiarioNome }}</span>
                    <span class="person-rg">RG {{ av.estagiarioRg }}</span>
                  </div>
                </td>
                <td class="col-center">
...
import { useAvaliacaoEstagioStore } from '@/stores/avaliacaoEstagio'
import { useToastStore } from '@/stores/toast'
import { useClock } from '@/composables/useClock'
import AppSidebar from '@/components/layout/AppSidebar.vue'
import AppTopbar from '@/components/layout/AppTopbar.vue'
import BaseTable from '@/components/ui/BaseTable.vue'
import CriarAvaliacaoModal from '@/components/gestao/CriarAvaliacaoModal.vue'

const auth = useAuthStore()
const store = useAvaliacaoEstagioStore()
...
const breadcrumb = computed(() => {
  if (isEstagio.value) return 'Estágio / Minhas Avaliações'
  if (isP1.value) return 'Gestão de Pessoal / Avaliações de Estágio'
  return 'Estágio / Avaliações Realizadas'
})

const cardSubtitle = computed(() => {
  if (isEstagio.value) return 'Avaliações que você recebeu durante o estágio.'
  if (isP1.value) return 'Todas as avaliações do efetivo em estágio.'
  return 'Avaliações de estágio que você realizou.'
})
...
function limparFiltros() {
  Object.assign(filters, { dataInicio: '', dataFim: '', nota_min: '', nota_max: '', avaliadorId: '', estagiarioId: '' })
}
